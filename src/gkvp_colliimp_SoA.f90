...
!< vp-mu
  ...
  real(kind=DP), save, &
    dimension(1:2*global_nv,0:global_nm,0:ns-1,-nz:nz-1) :: gnu_ds, gnu_ps, &
                                                            gnu_hs, gnu_gs
!!!- AoS (array of structure) layout - 
!!!  real(kind=DP), save, &
!!!    dimension(1:6,1:2*global_nv,0:global_nm,0:ns-1,0:ns-1,-nz:nz-1) &
!!!                                             :: gvfunc, gx_tst, gy_fld
!!!- SoA (structure of array) layout - 
  real(kind=DP), save, &
    dimension(1:2*global_nv,0:global_nm,0:ns-1,0:ns-1,-nz:nz-1,1:6) &
                                             :: gvfunc, gx_tst, gy_fld
...
!!!                end if
!!!- SoA (structure of array) layout - 
                gvfunc(iv,im,ia,ib,iz,1) = gc_t01 / gfmx(iv,im,iz)                          * cintgrl
                gvfunc(iv,im,ia,ib,iz,2) = gc_t01 / gfmx(iv,im,iz) * (gvp(im,iz) / gvl(iv)) * cintgrl
                gvfunc(iv,im,ia,ib,iz,3) = gc_t02 / gfmx(iv,im,iz)                          * cintgrl
                gvfunc(iv,im,ia,ib,iz,4) = gvl(iv)                                          * cintgrl
                gvfunc(iv,im,ia,ib,iz,5) = gvp(im,iz)                                       * cintgrl
                gvfunc(iv,im,ia,ib,iz,6) = (gxxa**2 - 1.5_DP)                               * cintgrl 

                gx_tst(iv,im,ia,ib,iz,1) = (ctheta(ia,ib) - 1._DP) * gfmx(iv,im,iz) * gvl(iv)
                gx_tst(iv,im,ia,ib,iz,2) = gx_tst(iv,im,ia,ib,iz,1) * gvp(im,iz) / gvl(iv) 
                gx_tst(iv,im,ia,ib,iz,3) = gx_tst(iv,im,ia,ib,iz,1) * (gxxa**2/1.5_DP - 1._DP) / gvl(iv)
                gx_tst(iv,im,ia,ib,iz,4) = (ctheta(ia,ib) - 1._DP) * (gc_t01                              &
                                            - (ctheta(ia,ib) - 1._DP) * calpha(ia,ib) * ctauiv(ia,ib)     &
                                              * gfmx(iv,im,iz) * gvl(iv) / dsqrt(1._DP + calpha(ia,ib)**2) )
                gx_tst(iv,im,ia,ib,iz,5) =  gx_tst(iv,im,ia,ib,iz,4) * gvp(im,iz) / gvl(iv)  
                gx_tst(iv,im,ia,ib,iz,6) = (ctheta(ia,ib) - 1._DP) * (gc_t02 * 2._DP/3._DP            &
                                            - (ctheta(ia,ib) - 1._DP) * calpha(ia,ib) * ctauiv(ia,ib) &
                                              * gfmx(iv,im,iz) * (gxxa**2/1.5_DP - 1._DP) * 2._DP     &
                                                / (1._DP + calpha(ia,ib)**2)**1.5 )

                if (fcs(ia) == 0.d0 .or. fcs(ib) == 0.d0) then !-care for tracer particle(dens=0)-
                  gy_fld(iv,im,ia,ib,iz,1:6) = 0._DP
                else
                  gy_fld(iv,im,ia,ib,iz,1) = - (fcs(ib)/Znum(ib)) / (fcs(ia)/Znum(ia)) * calpha(ia,ib) * Anum(ia)  & 
                                               * tau(ib) * ctheta(ia,ib) * ctheta(ib,ia) / tau(ia) / cgamma(ia,ib) &
                                                        * ( gc_t01 - cxi(ia,ib) * gfmx(iv,im,iz) * gvl(iv) ) 
                  gy_fld(iv,im,ia,ib,iz,2) = gy_fld(iv,im,ia,ib,iz,1) * gvp(im,iz) / gvl(iv)  
                  gy_fld(iv,im,ia,ib,iz,3) = - (fcs(ib)/Znum(ib)) / (fcs(ia)/Znum(ia))                          & 
                                                        * tau(ib) * ctheta(ia,ib) * ctheta(ib,ia) / ceta(ia,ib) &
                                                        * ( gc_t02                                          &
                                                            - cxi(ia,ib)/(1._DP+calpha(ia,ib)**2)*gfmx(iv,im,iz)&
                                                              *(2._DP*gxxa**2 - 3._DP) ) 
                  gy_fld(iv,im,ia,ib,iz,4) = - gy_fld(iv,im,ia,ib,iz,1)*cxi(ib,ia) 
                  gy_fld(iv,im,ia,ib,iz,5) = - gy_fld(iv,im,ia,ib,iz,2)*cxi(ib,ia) 
                  gy_fld(iv,im,ia,ib,iz,6) = - gy_fld(iv,im,ia,ib,iz,3)*2._DP*cxi(ib,ia)/(1._DP+calpha(ib,ia)**2) 
                end if
...
!!!- SoA (structure of array) layout - 
    complex(kind=DP), dimension(0:ns-1,0:ns-1,1:6) :: moment_ab
...
!!!- SoA (structure of array) layout - 
    complex(kind=DP), intent(out), &
      dimension(0:ns-1,0:ns-1,1:6) :: moment_ab
    integer, intent(in) :: iz, ibuff
      ...
      if ( iFLR == 1 ) then  ! full-GK
        ...
        do ib = 0, ns-1
          do ia = 0, ns-1
            ...
            do im = 1, global_nm
              do iv = 1, 2*global_nv
                  moment_ab(ia,ib,1) = moment_ab(ia,ib,1) + wf(iv,im,ia) * gj0(im,ia,iz,ibuff) * gvfunc(iv,im,ia,ib,iz,1)
                  moment_ab(ia,ib,2) = moment_ab(ia,ib,2) + wf(iv,im,ia) * gj1(im,ia,iz,ibuff) * gvfunc(iv,im,ia,ib,iz,2)
                  moment_ab(ia,ib,3) = moment_ab(ia,ib,3) + wf(iv,im,ia) * gj0(im,ia,iz,ibuff) * gvfunc(iv,im,ia,ib,iz,3)
                  moment_ab(ia,ib,4) = moment_ab(ia,ib,4) + wf(iv,im,ia) * gj0(im,ia,iz,ibuff) * gvfunc(iv,im,ia,ib,iz,4)
                  moment_ab(ia,ib,5) = moment_ab(ia,ib,5) + wf(iv,im,ia) * gj1(im,ia,iz,ibuff) * gvfunc(iv,im,ia,ib,iz,5)
                  moment_ab(ia,ib,6) = moment_ab(ia,ib,6) + wf(iv,im,ia) * gj0(im,ia,iz,ibuff) * gvfunc(iv,im,ia,ib,iz,6)
              end do
            end do
        ...
        do ib = 0, ns-1
          do ia = 0, ns-1
           ...
           !- edge compensation -
            im = 1
              do iv = 1, 2*global_nv
                wfvp  = wf(iv,im  ,ia) * gj0(im  ,ia,iz,ibuff) * gvfunc(iv,im  ,ia,ib,iz,1)
                wfvp1 = wf(iv,im+1,ia) * gj0(im+1,ia,iz,ibuff) * gvfunc(iv,im+1,ia,ib,iz,1)
                moment_ab(ia,ib,1) = moment_ab(ia,ib,1)        &
                                   - ( - wfvp / 12._DP         &
                                       + ( wfvp1               &
                                         - wfvp * 2._DP        &
                                         ) * 11._DP / 720._DP )
                wfvp  = wf(iv,im  ,ia) * gj1(im  ,ia,iz,ibuff) * gvfunc(iv,im  ,ia,ib,iz,2)
                wfvp1 = wf(iv,im+1,ia) * gj1(im+1,ia,iz,ibuff) * gvfunc(iv,im+1,ia,ib,iz,2)
                moment_ab(ia,ib,2) = moment_ab(ia,ib,2)        &
                                   - ( - wfvp / 12._DP         &
                                       + ( wfvp1               &
                                         - wfvp * 2._DP        &
                                         ) * 11._DP / 720._DP )
                wfvp  = wf(iv,im  ,ia) * gj0(im  ,ia,iz,ibuff) * gvfunc(iv,im  ,ia,ib,iz,3)
                wfvp1 = wf(iv,im+1,ia) * gj0(im+1,ia,iz,ibuff) * gvfunc(iv,im+1,ia,ib,iz,3)
                moment_ab(ia,ib,3) = moment_ab(ia,ib,3)        &
                                   - ( - wfvp / 12._DP         &
                                       + ( wfvp1               &
                                         - wfvp * 2._DP        &
                                         ) * 11._DP / 720._DP )
                wfvp  = wf(iv,im  ,ia) * gj0(im  ,ia,iz,ibuff) * gvfunc(iv,im  ,ia,ib,iz,4)
                wfvp1 = wf(iv,im+1,ia) * gj0(im+1,ia,iz,ibuff) * gvfunc(iv,im+1,ia,ib,iz,4)
                moment_ab(ia,ib,4) = moment_ab(ia,ib,4)        &
                                   - ( - wfvp / 12._DP         &
                                       + ( wfvp1               &
                                         - wfvp * 2._DP        &
                                         ) * 11._DP / 720._DP )
                wfvp  = wf(iv,im  ,ia) * gj1(im  ,ia,iz,ibuff) * gvfunc(iv,im  ,ia,ib,iz,5)
                wfvp1 = wf(iv,im+1,ia) * gj1(im+1,ia,iz,ibuff) * gvfunc(iv,im+1,ia,ib,iz,5)
                moment_ab(ia,ib,5) = moment_ab(ia,ib,5)        &
                                   - ( - wfvp / 12._DP         &
                                       + ( wfvp1               &
                                         - wfvp * 2._DP        &
                                         ) * 11._DP / 720._DP )
                wfvp  = wf(iv,im  ,ia) * gj0(im  ,ia,iz,ibuff) * gvfunc(iv,im  ,ia,ib,iz,6)
                wfvp1 = wf(iv,im+1,ia) * gj0(im+1,ia,iz,ibuff) * gvfunc(iv,im+1,ia,ib,iz,6)
                moment_ab(ia,ib,6) = moment_ab(ia,ib,6)        &
                                   - ( - wfvp / 12._DP         &
                                       + ( wfvp1               &
                                         - wfvp * 2._DP        &
                                         ) * 11._DP / 720._DP )
        ...
        do ib = 0, ns-1
          do ia = 0, ns-1
            ...
            do im = 1, global_nm
              do iv = 1, 2*global_nv
                  moment_ab(ia,ib,1) = moment_ab(ia,ib,1) + wf(iv,im,ia) * gvfunc(iv,im,ia,ib,iz,1)
                  moment_ab(ia,ib,3) = moment_ab(ia,ib,3) + wf(iv,im,ia) * gvfunc(iv,im,ia,ib,iz,3)
                  moment_ab(ia,ib,4) = moment_ab(ia,ib,4) + wf(iv,im,ia) * gvfunc(iv,im,ia,ib,iz,4)
                  moment_ab(ia,ib,6) = moment_ab(ia,ib,6) + wf(iv,im,ia) * gvfunc(iv,im,ia,ib,iz,6)
              end do
            end do
        ...
        do ib = 0, ns-1
          do ia = 0, ns-1
           ...
           !- edge compensation -
            im = 1
              do iv = 1, 2*global_nv
                wfvp  = wf(iv,im  ,ia) * gvfunc(iv,im  ,ia,ib,iz,1)
                wfvp1 = wf(iv,im+1,ia) * gvfunc(iv,im+1,ia,ib,iz,1)
                moment_ab(ia,ib,1) = moment_ab(ia,ib,1)        &
                                   - ( - wfvp / 12._DP         &
                                       + ( wfvp1               &
                                         - wfvp * 2._DP        &
                                         ) * 11._DP / 720._DP )
                wfvp  = wf(iv,im  ,ia) * gvfunc(iv,im  ,ia,ib,iz,3)
                wfvp1 = wf(iv,im+1,ia) * gvfunc(iv,im+1,ia,ib,iz,3)
                moment_ab(ia,ib,3) = moment_ab(ia,ib,3)        &
                                   - ( - wfvp / 12._DP         &
                                       + ( wfvp1               &
                                         - wfvp * 2._DP        &
                                         ) * 11._DP / 720._DP )
                wfvp  = wf(iv,im  ,ia) * gvfunc(iv,im  ,ia,ib,iz,4)
                wfvp1 = wf(iv,im+1,ia) * gvfunc(iv,im+1,ia,ib,iz,4)
                moment_ab(ia,ib,4) = moment_ab(ia,ib,4)        &
                                   - ( - wfvp / 12._DP         &
                                       + ( wfvp1               &
                                         - wfvp * 2._DP        &
                                         ) * 11._DP / 720._DP )
                wfvp  = wf(iv,im  ,ia) * gvfunc(iv,im  ,ia,ib,iz,6)
                wfvp1 = wf(iv,im+1,ia) * gvfunc(iv,im+1,ia,ib,iz,6)
                moment_ab(ia,ib,6) = moment_ab(ia,ib,6)        &
                                   - ( - wfvp / 12._DP         &
                                       + ( wfvp1               &
                                         - wfvp * 2._DP        &
          ...
          do ib = 0, ns-1
            do ia = 0, ns-1
              do im = 0, global_nm
                do iv = 1, 2*global_nv
                  cf(iv,im,ia) = cf(iv,im,ia)             &
                               + gx_tst(iv,im,ia,ib,iz,1) &
                                * moment_ab(ia,ib,1)      &
                                 * gj0(im,ia,iz,ibuff)    & 
                               + gx_tst(iv,im,ia,ib,iz,2) &
                                * moment_ab(ia,ib,2)      &
                                 * gj1(im,ia,iz,ibuff)    & 
                               + gx_tst(iv,im,ia,ib,iz,3) &
                                * moment_ab(ia,ib,3)      &
                                 * gj0(im,ia,iz,ibuff)    & 
                               + gx_tst(iv,im,ia,ib,iz,4) &
                                * moment_ab(ia,ib,4)      &
                                 * gj0(im,ia,iz,ibuff)    & 
                               + gx_tst(iv,im,ia,ib,iz,5) &
                                * moment_ab(ia,ib,5)      &
                                 * gj1(im,ia,iz,ibuff)    & 
                               + gx_tst(iv,im,ia,ib,iz,6) &
                                * moment_ab(ia,ib,6)      &
                                 * gj0(im,ia,iz,ibuff)
                end do
              end do
          ...
          do ib = 0, ns-1
            do ia = 0, ns-1
              do im = 0, global_nm
                do iv = 1, 2*global_nv
                  cf(iv,im,ia) = cf(iv,im,ia)             &
                               + gy_fld(iv,im,ia,ib,iz,1) &
                                * moment_ab(ib,ia,1)      &
                                 * gj0(im,ia,iz,ibuff)    & 
                               + gy_fld(iv,im,ia,ib,iz,2) &
                                * moment_ab(ib,ia,2)      &
                                 * gj1(im,ia,iz,ibuff)    & 
                               + gy_fld(iv,im,ia,ib,iz,3) &
                                * moment_ab(ib,ia,3)      &
                                 * gj0(im,ia,iz,ibuff)    & 
                               + gy_fld(iv,im,ia,ib,iz,4) &
                                * moment_ab(ib,ia,4)      &
                                 * gj0(im,ia,iz,ibuff)    & 
                               + gy_fld(iv,im,ia,ib,iz,5) &
                                * moment_ab(ib,ia,5)      &
                                 * gj1(im,ia,iz,ibuff)    & 
                               + gy_fld(iv,im,ia,ib,iz,6) &
                                * moment_ab(ib,ia,6)      &
                                 * gj0(im,ia,iz,ibuff)
                end do
              end do
